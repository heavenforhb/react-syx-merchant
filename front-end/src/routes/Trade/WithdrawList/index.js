...
import { WITHDRAW_STATUS, TRAN_TYPE, BUS_TYPE } from '../../../common/constants'
...
class WithdrawList extends Component {
  ...
  refresh = () => {
    ...
    dispatch({ type: 'withdrawList/fetch', payload: this.state.query })
  }
  ...
  queryDetail = (orderNo) => {
    ...
    if (loading) return false
    dispatch({ type: 'withdrawList/detail', payload: { orderNo, mercCd: currentUser.mercCd } })
  }
  ...
  render() {
    ...
    const { orderNo, payOrder, bankBillNo, orderDt, orderTm, payTm, bankName, busType, mercCd, witOrderSts, 
      orderAmt, orderRmk, bankCd, recFeeAmt, oprCd
    } = detail
    ...
    const showDetail = [
      {
        ...
        key: '业务类型',
        val: busType ? BUS_TYPE['1005'][busType] : '-'
      }, {
        key: '提现状态',
        val: witOrderSts ? WITHDRAW_STATUS[witOrderSts].name : '-'
      }, {
        key: '商户编码',
        val: mercCd ? mercCd : '-'
      }, {
    ...
    return (
      <PageHeaderLayout
        title="提现记录"
      >
        <Modal visible={detailModal} width={800} onCancel={() => dispatch({ type: 'withdrawList/detailModalFlag', payload: { show: false } })} title='订单详情' footer={
          <Button type='primary' size='large' onClick={() => dispatch({ type: 'withdrawList/detailModalFlag', payload: { show: false } })}>关闭</Button>}>
        ...
        <Modal visible={downloadModal} width={800} destroyOnClose={true} maskClosable={false} onCancel={() => this.setState({ downloadModal: false })} title='订单详情' footer={null}>
          <DownloadComponent query={query} madeFileUrl={'/api/download/withdrawList/madeFile'} downloadUrl={'/api/download/posp/createUrl'} cancel={() => this.setState({ downloadModal: false })} />
        </Modal>
        <Card bordered={false} title="查询条件">
          <Row gutter={24}>
            <Col {...colProps} className={styles.col}>
              <span className={styles.inputLabel}>提现状态：</span>
              <Select className={styles.select} value={query.witOrderSts} onChange={val => this.queryChange('witOrderSts', val)}>
                <Select.Option value=''>全部</Select.Option>
                {Object.keys(WITHDRAW_STATUS).map(key => <Select.Option value={key} key={key}>{WITHDRAW_STATUS[key]['name']}</Select.Option>)}
              </Select>
            </Col>
            <Col {...colProps} className={styles.col}>
          ...
          <Row>
            <Col xs={0} md={0} lg={24}>
              <Table columns={columns} dataSource={(list || []).map((item, idx) => {
                const { orderDt, orderTm, orderAmt, witOrderSts, recFeeAmt, orderNo, bankName, busType } = item
                const status = WITHDRAW_STATUS[witOrderSts]
                return {
                  key: idx,
                  orderTime: formatTime(orderDt, orderTm),
                  orderAmt: fMoney(orderAmt),
                  recFeeAmt: fMoney(recFeeAmt),
                  busType: BUS_TYPE['1005'][busType],
                  bankName,
                  orderNo,
            ...
            <Col xs={24} md={24} lg={0}>
              <Table columns={[
                ...
                const { orderNo, witOrderSts } = item
                const status = WITHDRAW_STATUS[witOrderSts]
                return {
                  key: idx,
                  orderNo,
...
function mapStateToProps(state) {
  const { loading, list, total, statistics, detail, detailModal } = state.withdrawList
