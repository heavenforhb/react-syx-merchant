...
import { CHARGE_STATUS, TRAN_TYPE, BUS_TYPE } from '../../../common/constants'
...
class ChargeList extends Component {
  ...
  refresh = () => {
    ...
    dispatch({ type: 'chargeList/fetch', payload: this.state.query })
  }
  ...
  queryDetail = (orderNo) => {
    ...
    if (loading) return false
    dispatch({ type: 'chargeList/detail', payload: { orderNo, mercCd: currentUser.mercCd } })
  }
  ...
  render() {
    ...
    const { orderNo, payOrder, bankBillNo, orderDt, orderTm, tranTm, payTm, 
      bankName, busType, errCodeDesc, orderAmt, recOrderSts, bankCd, oprCd, orderRmk
    } = detail
    ...
    const showDetail = [
      {
        ...
        key: '业务类型',
        val: busType ? BUS_TYPE['1004'][busType] : '-'
      }, {
        key: '充值状态',
        val: recOrderSts ? CHARGE_STATUS[recOrderSts].name : '-'
      }, {
        key: '金额',
    ...
    return (
      <PageHeaderLayout
        title="充值记录"
      >
        <Modal visible={detailModal} width={800} onCancel={() => dispatch({ type: 'chargeList/detailModalFlag', payload: { show: false } })} title='订单详情' footer={
          <Button type='primary' size='large' onClick={() => dispatch({ type: 'chargeList/detailModalFlag', payload: { show: false } })}>关闭</Button>}>
        ...
        <Modal visible={downloadModal} width={800} destroyOnClose={true} maskClosable={false} onCancel={() => this.setState({ downloadModal: false })} title='订单详情' footer={null}>
          <DownloadComponent query={query} madeFileUrl={'/api/download/chargeList/madeFile'} downloadUrl={'/api/download/posp/createUrl'} cancel={() => this.setState({ downloadModal: false })} />
        </Modal>
        <Card bordered={false} title="查询条件">
          <Row gutter={24}>
            <Col {...colProps} className={styles.col}>
              <span className={styles.inputLabel}>充值状态：</span>
              <Select className={styles.select} value={query.recOrderSts} onChange={val => this.queryChange('recOrderSts', val)}>
                <Select.Option value=''>全部</Select.Option>
                {Object.keys(CHARGE_STATUS).map(key => <Select.Option value={key} key={key}>{CHARGE_STATUS[key]['name']}</Select.Option>)}
              </Select>
            </Col>
            <Col {...colProps} className={styles.col}>
              ...
              <Select className={styles.select} value={query.busType} onChange={val => this.queryChange('busType', val)}>
                <Select.Option value=''>全部</Select.Option>
                {Object.keys(BUS_TYPE['1004']).map(key => <Select.Option value={key} key={key}>{BUS_TYPE['1004'][key]}</Select.Option>)}
              </Select>
            </Col>
          ...
          <Row>
            <Col xs={0} md={0} lg={24}>
              <Table columns={columns} dataSource={(list || []).map((item, idx) => {
                const { orderDt, orderTm, orderAmt, recOrderSts, orderNo, bankName, busType } = item
                const status = CHARGE_STATUS[recOrderSts]
                return {
                  key: idx,
            ...
            <Col xs={24} md={24} lg={0}>
              <Table columns={[
                ...
                const { orderNo, recOrderSts } = item
                const status = CHARGE_STATUS[recOrderSts]
                return {
                  key: idx,
                  orderNo,
...
function mapStateToProps(state) {
  const { loading, list, total, statistics, detail, detailModal } = state.chargeList
