import * as subOrderService from '../services/subOrder'
...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put }) {
      ...
      try {
        const { dataList, dataStatistics, totalRecord } = yield call(subOrderService.fetch, payload)
        yield put({
          type: 'save',
          payload: {
            list: dataList,
    ...
    *detail({ payload }, { call, put }) {
      yield put({ type: 'loading', payload: { show: true } })
      try {
        const detail = yield call(subOrderService.detailFetch, payload)
        yield put({
          type: 'detailData',
          payload: {
            detail
