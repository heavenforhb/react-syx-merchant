import * as paymentListService from '../services/paymentList'
...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put }) {
      ...
      try {
        const { dataList, dataStatistics, totalRecord } = yield call(paymentListService.fetch, payload)
        yield put({
          type: 'save',
          payload: {
            list: dataList,
    ...
    *detail({ payload }, { call, put }) {
      yield put({ type: 'loading', payload: { show: true } })
      try {
        const detail = yield call(paymentListService.detailFetch, payload)
        yield put({
          type: 'detailData',
          payload: {
            detail
