import * as withdrawListService from '../services/withdrawList'
...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put }) {
      ...
      try {
        const { dataList, dataStatistics, totalRecord } = yield call(withdrawListService.fetch, payload)
        yield put({
          type: 'save',
          payload: {
            list: dataList,
    ...
    *detail({ payload }, { call, put }) {
      yield put({ type: 'loading', payload: { show: true } })
      try {
        const detail = yield call(withdrawListService.detailFetch, payload)
        yield put({
          type: 'detailData',
          payload: {
            detail
